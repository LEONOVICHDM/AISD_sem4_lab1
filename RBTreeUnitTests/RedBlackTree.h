...
public:
	...
	void print();
	void getValues(list<T>& valueList);
	void getKeys(list<T>& keyList);
	void clear();
	int isEmpty();
...
private:
	...
	void getValues(RedBlackTreeNode<T>* tree, list<T>& valueList);
	void getKeys(RedBlackTreeNode<T>* tree, list<T>& keyList);
private:
...
template<class T>
void RedBlackTree<T>::getValues(list<T>& valueList) {
	if (root == NULL)
		cout << "empty RBtree\n";
	else
		getValues(root, valueList);
};

template<class T>
void RedBlackTree<T>::getValues(RedBlackTreeNode<T>* tree, list<T>& valueList) {
	if (tree != NULL) {
		valueList.push_back(tree->value);
		getValues(tree->left, valueList);
		getValues(tree->right, valueList);
	}
}

template<class T>
void RedBlackTree<T>::getKeys(list<T>& keyList) {
	if (root == NULL)
		cout << "empty RBtree\n";
	else
		getKeys(root, keyList);
};

template<class T>
void RedBlackTree<T>::getKeys(RedBlackTreeNode<T>* tree, list<T>& keyList) {
	if (tree != NULL) {
		keyList.push_back(tree->key);
		getKeys(tree->left, keyList);
		getKeys(tree->right, keyList);
	}
}
